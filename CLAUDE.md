...
- **Remote calibration (`calibration.json` at the repo root).** The `pb`/proto
  ...
  **splitting the utterance on sentence boundaries and splicing silence in-app**
  ...
  the neural voice, "152nd" came out sounding like "one fifth second" on a real drive, 2026-07-07) are
...
- **Surveillance-camera (Flock / ALPR) layer (`OverpassAlprCameras` + `refreshFlock` + `FLOCK_LAYER`, device-verified
  ...
  Proven: `overpass-api.de` was 504-ing while `maps.mail.ru` returned 16 Flock nodes over the same the test suburb box.
  ...
  **BUNDLED on-device dataset (2026-07-13, supersedes the live Overpass path for cameras):** the whole
  ...
  the test suburb & the state route with no network wait. NB "avoid" still only RE-RANKS the alternates Google/OSRM
...
- **Tap-through route stop timeline (2026-07-12, keyless + device-verified).** Every `DepartureLineRow`
  ...
  reviewed old shelter was beating the live stop - the "a suburban junction shows Permanently
