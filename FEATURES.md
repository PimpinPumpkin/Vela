...
- 🟡 **Open places layer, beta (2026-09-14, issue #441).** Settings, Map, "Open places layer (beta)": where a region file covers the view, the businesses on the map come from open data (Overture Places baked to PMTiles by `tools/build-places-region.sh`, drawn with the same icons, sizes and label thresholds as the Google dots) instead of the fetch that used to fire as you pan. Google is asked only when you tap a place, through the same name-and-distance correlation a basemap tap uses; offline, the tile's own category, address, phone and website fill the sheet. Test region: Davis, streamed from the `places-overlays` release or downloaded along with a region for offline use; a second tap on the same pin is instant. Parks, schools, civic and transit stay with OSM
