...
- ✅ **Traversed route greys out** behind the vehicle (Google-style) — a line-progress gradient on the route (grey for the part driven, congestion-tinted ahead); **live traffic auto-shows during navigation** (the `/maps/vt` overlay layers above the route so congestion reads on your line). Nav-overlay polish: speedometer + recenter lifted clear of the bottom bar, recenter is now an icon-only FAB tucked bottom-right. **Maneuver-banner swipe fixed** — `pointerInput(Unit)` had captured a stale step index so every swipe repeated one card; `rememberUpdatedState` makes each swipe walk to the next/prev step
