...
- ⚠️ **Place photos — keyless preview only (Google gated the gallery, 2026-06).** The photo strip **leads the sheet as a hero** (horizontally scrollable; **tap → full-screen, swipeable gallery** with a counter). Photos come from the search response: the hero block at `[1][72][0][i][6][0]` *(Google moved it `[105]`→`[72]` on 2026-06-27, blanking every strip; hot-fixed via calibration `v7`)* **de-duped** (Google now serves the single hero photo **twice**), plus a small `[1][204][0][i][1][2][0][0]` "gallery preview" that **only big/landmark places carry** (Space Needle → ~4; an ordinary business → **1**). **The full ~30-photo gallery is now login-gated** — the keyless `hspqX` RPC returns only Street View thumbnails to an anonymous session (filtered out), so the old "all photos aggregate at the top" is no longer reachable keyless. Same wall as keyless reviews (~3) and popular-times. *(Full gallery + the posted-date viewer `entry[21][6][8]` still work behind an opt-in Google sign-in — see ROADMAP.)*
