...
- ✅ **Place photos — keyless (preview always; full gallery intermittently, 2026-06).** The photo strip **leads the sheet as a hero** (horizontally scrollable; **tap → full-screen, swipeable gallery** with a counter). Photos come from the search response: the hero block at `[1][72][0][i][6][0]` *(Google moved it `[105]`→`[72]` on 2026-06-27, blanking every strip; hot-fixed via calibration `v7`)* **de-duped** (Google now serves the single hero photo **twice**), plus a small `[1][204][0][i][1][2][0][0]` "gallery preview" that **only big/landmark places carry** (Space Needle → ~4; an ordinary business → **1**). The **full ~30-photo gallery is reachable keyless but UNRELIABLE** (corrected 2026-06-28 — *not* login-gated as first thought, but not the retry-fixable "transient" I then guessed either): on-device logging showed the anonymous-WebView `hspqX` RPC gets a **degraded Street-View-only reply per-SESSION** (4 retries in 5 s were byte-identical), so a same-session retry can't recover it. When Google *doesn't* degrade the session, the **real user gallery** comes back and swaps in (observed live — a place populated its full gallery a few seconds after opening); we just can't reliably force that state. `WebPhotoFetcher` keeps a tiny 2× hedge and falls back to the de-duped preview otherwise. The promising un-degrade lever (load the place's own page so Google renders photos in-context, like the reviews scrape) is on the ROADMAP. Posted-date in the viewer from `entry[21][6][8]`.
