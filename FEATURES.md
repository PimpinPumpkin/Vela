...
- ✅ **Surveillance-camera (Flock / ALPR) layer (2026-07-12, user request).** Settings > Map > **Surveillance cameras** draws mapped automated licence-plate readers on the map - the community **DeFlock** project's `surveillance:type=ALPR` nodes in OpenStreetMap (Flock Safety, Motorola, police plate readers) - as a distinct purple camera badge at close zoom. Keyless (Overpass), fetched per-viewport and **area-cached** exactly like the traffic-controls layer (`OverpassAlprCameras`, `refreshFlock`, `FLOCK_LAYER`), no account and nothing to do with Flock or Google. **OFF by default** (`Flock` holder), a privacy-awareness layer you opt into; toggling it on refetches the current view immediately. Coverage is OSM's/DeFlock's - strong across the US, growing elsewhere. **Warn about cameras on route (2026-07-12, opt-in):** a SEPARATE Settings > Map toggle ("Warn about cameras on route", OFF by default, `FlockRouteAlert`) badges each route option on the picker with **how many mapped cameras it passes** (amber camera glyph + "N cameras on this route"), so you can pick a route that dodges them - it pairs with long-press "route through here" to actually steer around a camera stretch. Keyless: `OverpassAlprCameras.fetchAlong` counts the ALPR nodes within ~120 m of each route's polyline (reusing the `out body` fetch), off the hot path, index-aligned with the route list. **Device-verified: a route through a camera-dense corridor showed "18 cameras" / "19 cameras" while the alternate via a different road showed none.** Kept independent of the map layer on purpose (you might want one without the other). **Device-verified 2026-07-12 in Atlanta** (Ponce City Market): the purple camera badges draw on the map. Two bugs were caught and fixed in that verification: the Overpass query used `out tags`, which for a node returns id + tags but **no lat/lon**, so the parser dropped every camera and the layer was always empty - now `out body`; and `Flock.init` was missing from `VelaApp`, so the toggle **reset to off on every app restart** (the persisted pref was never loaded) - now initialised alongside the other layer holders. *(Tap-for-detail on a camera and an approach chime are possible follow-ups; this ships the map layer.)*
...
- ✅ **Full reviews - now via a hidden WebView, WITH per-review photos (2026-06-27).** Google
  **deleted** the keyless `listentitiesreviews` endpoint (it now 404s) and moved reviews behind a
  ...
  below). On-device-verified (a local park: real review text + photo rows of swings/lake/ducks).
