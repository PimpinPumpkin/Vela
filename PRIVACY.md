...
| Every pan and zoom of the map | Yes - their servers render the map | Yes | Map tiles come from OpenFreeMap, so Google never renders your view. With "Places come from" on its default, Both, one anonymous places request carrying the map area goes to Google after you stop panning, to add businesses Vela's own data lacks; set it to Vela data (Settings > Data & privacy) and browsing never touches Google |
