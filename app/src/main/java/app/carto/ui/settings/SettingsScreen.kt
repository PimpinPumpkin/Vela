...
import app.carto.ui.map.MapViewModel
...
fun SettingsScreen(vm: MapViewModel, onBack: () -> Unit) {
    ...
    Scaffold(
        ...
        Column(
            ...
            if (engines.isEmpty()) {
                Hint("No TTS engine detected yet. On a degoogled ROM, install RHVoice or eSpeak NG from F-Droid for natural voices — Carto will list them here.")
            }
            engines.forEach { e ->
                SelectableRow(
                    label = e.label,
            ...
            Hint(
                "Carto currently runs on built-in mock data so the whole app works offline. " +
                    "The Google extractor switches on once its request/response shapes are " +
                    "calibrated (CartoConfig.USE_GOOGLE_SOURCE). No Google account or API key " +
            ...
            SectionTitle("About")
            Hint(
                "Carto is a degoogled maps client: open tiles for the basemap, scraped Google " +
