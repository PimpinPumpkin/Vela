...
class WebPopularTimesFetcher @Inject constructor(
...
) : HiddenWebView(context, "popular") {
    @Volatile private var warm: CompletableDeferred<Unit>? = null
    private var warmFinishes = 0
    ...
    suspend fun fetch(place: Place): PlaceDetails? {
        ...
        // A *specific* query (name + address) is essential: a bare-name search comes back as a
        // 20-result [64] list trimmed of [84], while name+address resolves to the single focused
        // result that keeps the histogram. Thread it through BOTH the pb's !1s query block and the
        // q= param so they agree.
        ...
        val raw = session {
            request(TOTAL_TIMEOUT_MS) { id ->
                ensureWarm()
                evaluate(script(id, url))
            }
        }
        val d = if (raw.isNullOrEmpty()) null
        else runCatching { PopularTimesParser.parse(raw, place.featureId, cal.paths) }.getOrNull()
        Log.i(TAG, "popular: raw=${raw?.length ?: -1} parsed=${d != null} times=${d?.popularTimes != null}")
        return d
    }
    ...
    suspend fun prewarm() {
        runCatching { withTimeoutOrNull(MAX_WARM_MS + 2_000L) { session { ensureWarm() } } }
    }

    /** Warm google.com -> maps once. The two-step establishes a real NID; a freshly
     *  consent-seeded session still gets a degraded (histogram-less) search. The page-finish
     *  callback drives it: first finish loads the maps warm URL, the second settles it. */
    private suspend fun ensureWarm() = withContext(Dispatchers.Main) {
        ...
        w.await()
        // The surrounding session() pauses the view once the fetch is done.
    }
    ...
    override fun onReaped() {
        warm = null // the next fetch re-runs the warm sequence
    }
