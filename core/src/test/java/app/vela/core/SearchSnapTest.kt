...
class SearchSnapTest {
     ...
     * real response - 8 related salons for "Bellagio Nails Sacramento".) */
    @Test fun parsesPeopleAlsoSearchFor() {
        val geo = arr(7, 0 to "[null,null,47.95,-122.21]", 6 to "4.7")     // [0]=coords, [6]=rating
        ...
        assertEquals(47.95, sims[0].location.lat, 1e-9)
        assertEquals(-122.21, sims[0].location.lng, 1e-9)
    }
    ...
    @Test fun enrichmentLiftsCountAddressRatingFromFocusedNode() {
        ...
        val geo = arr(4, 2 to "47.95", 3 to "-122.21")             // [9][2]/[9][3]=lat/lng
        val place = arr(40, 4 to rating, 9 to geo, 10 to "\"0xAAA:0xBBB\"",
            11 to "\"Test Spa\"", 39 to "\"123 Main St, Sacramento, WA\"")
        val z = arr(15, 14 to place)                                // single result at [0][1][0][14]
        val x = arr(2, 1 to "[$z]")
        val body = ")]}'\n[$x]"
        ...
        assertEquals("123 Main St, Sacramento, WA", d.address)
