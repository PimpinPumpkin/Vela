...
| **Offline routing** | On-device **OsmAnd obf** region files (OsmAnd's router, vendored), one per region, downloaded from a 414-piece world catalog; the speed-limit badge and romanized road names read off the same files | `core/data/ObfRouteEngine.kt`, `app/offline/ObfStore.kt`, `tools/routing-regions.json` |
| **Offline address geocoding** | Typed street address → coordinate → on-device route with no signal; keyless OSM `addr:housenumber` + named-road centrelines indexed on area download (house-precise, interpolated, or street-level fallback) | `core/data/OfflineAddressStore.kt`, `core/data/OverpassPois.kt` |
...
| **Open building overlay** | Microsoft building footprints (ODbL) as per-region PMTiles, rendered beneath OSM to fill areas OSM never mapped; world catalog covering every US state and most countries (US + Global ML sources) | `app/offline/OverlayTileStore.kt`, `app/ui/map/VelaMapView.kt`, `scripts/build-overlay-region.sh`, `tools/overlay-regions.json` |
...
| **Stop icons on the map** | **Transitous stop positions** (canonical GTFS) at street zoom; the OSM basemap icons stay wherever Transitous has no coverage. Tapping an icon opens the board directly by stop id. | GTFS positions come from the agencies themselves; OSM stop data is community-maintained and sometimes marks stops agencies do not serve. The direct stop-id tap also skips the fragile name matching between map data and Google listings. Same-named curb stops (the two sides of the street at an intersection) draw as one icon whose board carries both directions; the headsigns tell them apart, and transit directions still walk you to the exact boarding coordinate. Stops named per direction stay separate. |
| **Route stop list** (tap a route on the board) | The GTFS trip's own stop sequence (Transitous `trip` lookup by the run you tapped), with the Google itinerary as fallback. | The trip data is exact - the actual run you tapped, every stop it makes, with realtime times per stop - where the itinerary reuse guesses at a matching leg. |
