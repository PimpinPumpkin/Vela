...
phone never holds two copies of a region (see ROADMAP). What the producer cannot do is decide where
anything lands: the archive on the phone may have taken patches already, or been compacted, so its
tiles are not where this bake's copy of the old file had them.

Format, all little endian:

    "VELAPTCH" u8 version
    u32 header_len, header (utf-8 json)
    u32 plan_len,   the new directory minus its offsets, gzipped varint columns
    tile blobs, concatenated, in the order of header["tiles"]

The plan is the part worth understanding. An earlier version shipped the finished directory, which
meant the patch only applied to a byte-exact copy of the archive this bake diffed against: the
second patch a phone was offered always bounced, because the first one had already changed the
file's layout. So the patch does not name offsets at all. Each entry says either "the tile you
already have under this id" or "the next blob in this patch", and the applier resolves that against
its own file, whatever shape it is in.

The header carries what the applier needs to prove the result (`toFingerprint`, a hash over the new
archive's sorted tile ids and tile hashes). A fresh download of the same rev has the same
fingerprint, which is the acceptance criterion: an archive that has taken patches must be
...
from velapmtiles import (  # the one reader, so producer and applier cannot drift
    MAGIC, VERSION, blob, entries_of, fingerprint, write_plan,
)
...
def main():
    ...
    with open(old_p, "rb") as fo, open(new_p, "rb") as fn:
        ...
        # bytes changed; everything else it already has, under the same id.
        carried, payload, dir_entries, flags = [], bytearray(), [], []
        for tid, off, ln, run in new:
            nb = blob(fn, nh["tile_off"], off, ln)
            h = hashlib.sha256(nb).digest()
            keep = tid in old_hashes and old_hashes[tid] == h
            dir_entries.append((tid, 0, len(nb), run))
            flags.append(0 if keep else 1)
            if not keep:
                carried.append({"id": tid, "len": len(nb)})
                payload += nb

    plan = write_plan(dir_entries, flags, oh["internal"])
    head = {
        "version": VERSION,
        "fromRev": rev_from, "toRev": rev_to,
        "fromFingerprint": fingerprint(old_p, old, oh["tile_off"]),
    ...
    with open(out_p, "wb") as f:
        ...
        f.write(struct.pack("<I", len(plan))); f.write(plan)
        f.write(payload)
    ...
    print(f"{len(carried)} of {len(new)} tiles carried, patch {size / 1e6:.2f} MB "
          ...
          f"plan {len(plan) / 1e6:.2f} MB, dead {head['deadBytes'] / 1e6:.2f} MB")
